...
const cors = Cors({
  methods: ['Comment', 'GET', 'DELETE', 'PUT'],
})
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const handleCase: ResponseFuncs = {
    ...
    GET: async (req: NextApiRequest, res: NextApiResponse<CommentType>) => {
      await connect() // connect to database
      res.json(await Comment.findById(id).catch(catcher))
    },
    // RESPONSE PUT REQUESTS
    PUT: async (req: NextApiRequest, res: NextApiResponse<CommentType>) => {
      await connect() // connect to database
      res.json(
        await Comment.findByIdAndUpdate(id, req.body, { new: true }).catch(catcher)
      )
    },
